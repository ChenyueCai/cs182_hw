...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for i in range(H_out):
        for j in range(W_out):
            h_start, w_start = stride * i, stride * j
            h_end, w_end = h_start + HH, w_start + WW
            for k in range(F):        
                out[:, k, i, j] = np.sum(x_padded[:, :, h_start:h_end, w_start:w_end]* w[k], axis = (1,2,3))
...
def conv_backward_naive(dout, cache):
    ...
    for i in range(H_out):
        for j in range(W_out):
            ...
            x_pad_masked = x_pad[:, :, h_start:h_start+HH, w_start:w_start+WW]
            for k in range(F): #dw
                dw[k ,: ,: ,:] += np.sum(x_pad_masked * (dout[:, k, i, j])[:, None, None, None], axis=0)
            for n in range(N): #dx
                dx_pad[n, :, h_start:h_start+HH, w_start:w_start+WW] += np.sum((w *
...
def max_pool_forward_naive(x, pool_param):
    ...
    HH, WW = int(H/stride), int(W/stride)
    out = np.zeros((N,C,HH, WW))
    
    for i in range(HH):
        for j in range(WW):
            h_start, w_start = stride * i, stride * j
            mask = x[:,:, h_start : h_start + pool_height, w_start : w_start + pool_width]
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(out_height):
        for j in range(out_width):
            h_start, w_start = i*stride, j*stride
            x_mask = x[:, :, h_start:h_start+pool_height, w_start:w_start+pool_width]
            dx_mask = dx[:, :, h_start:h_start+pool_height, w_start:w_start+pool_width]
            grad_mask = np.max(x_mask, axis=(2, 3), keepdims=True) == x_mask
            dx_mask += grad_mask * (dout[:, :, i, j])[:, :, None, None]
